function [Classification, trainClass] = fun_belief_norm(Data, Priors, Mu, Sigma, samp_freq, epsilon)
    ...
    for k = 1:length(Data)
        ...
        dt = samp_freq;  % frequency of data 50 Hz for EPFL, 10 Hz for QMUL
        dataProcess = [];
...
%         [~ , ~, dataProcess, index] = preprocess_demos(Emp3Dnorm, 0.02, 0.0001); 
        ...
        b2_d = 0;
        b_d = [b1_d, b2_d];
        %epsilon = epsilon; % adaptation rate
