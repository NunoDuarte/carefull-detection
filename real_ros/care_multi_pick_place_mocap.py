...
class CareMultiPickandPlaceMocap:
    ...
    def __init__(self):
        ...
        self.coke = rospy.Subscriber("/Cup_1/pose", PoseStamped, self.tf_coke, queue_size=1)
        self.trans = rospy.Subscriber("/Red_cup/pose", PoseStamped, self.tf_trans, queue_size=1)
        self.gripperMsg = GripperCommandActionGoal()

        print('Starting node..')
        rospy.sleep(1)

        self.epsilon = 1  # adaptation rate
    ...
    def second_cup(self, ):
        ...
        act = False
        while not close:
            if active_coke and active_robot:
                coke = self.coke.pose.position

                coke_pose = np.zeros(3)
                coke_pose[0] = np.abs(np.round(coke.x - self.coke_initial.x, 3))
                coke_pose[1] = np.abs(np.round(coke.y - self.coke_initial.y, 3))
                coke_pose[2] = np.abs(np.round(coke.z - self.coke_initial.z, 3))
                # compute the vector distance
                coke_shift = float(math.sqrt(coke_pose[0]**2 + coke_pose[1]**2 + coke_pose[2]**2))

                if len(str(self.coke.header.stamp.nsecs)) == 7:
                    time = decimal.Decimal(str(self.coke.header.stamp.secs) + ".00" + str(self.coke.header.stamp.nsecs))
                elif len(str(self.coke.header.stamp.nsecs)) == 8:
                    time = decimal.Decimal(str(self.coke.header.stamp.secs) + ".0" + str(self.coke.header.stamp.nsecs))
                else:
                    time = decimal.Decimal(str(self.coke.header.stamp.secs) + "." + str(self.coke.header.stamp.nsecs))
                diff_time = float(time - self.time_initial)
                if not act:
                    print('time', diff_time)
                ...
                # get at least one past point
                if i > 1:
                    vel = np.abs((coke_shift - coke_past_shift) / (diff_time - coke_past_time))
                    if not act:
                        print('velocity', vel)
                    ...
                    if act and vel == 0.0 and coke_pose[2] < 0.03:
                        # if you have one option go to robot
                        if B[-1][0][0] > 0.8:
                            0 #self.robot_not([-0.2, 0.02, 0.05])
                        elif B[-1][0][1] > 0.8:
                            self.robot_careful([-0.2, +0.04, -0.02], active_coke, self.coke)
                        close = True

                coke_past_shift = coke_shift
                coke_past_time = diff_time
    ...
    def first_cup(self, ):
        ...
        act = False
        while not close:
            if active_trans and active_robot:
                trans = self.trans.pose.position

                trans_pose = np.zeros(3)
                trans_pose[0] = np.abs(np.round(trans.x - self.trans_initial.x, 3))
                trans_pose[1] = np.abs(np.round(trans.y - self.trans_initial.y, 3))
                trans_pose[2] = np.abs(np.round(trans.z - self.trans_initial.z, 3))
                # compute the vector distance
                trans_shift = float(math.sqrt(trans_pose[0] ** 2 + trans_pose[1] ** 2 + trans_pose[2] ** 2))

                if len(str(self.trans.header.stamp.nsecs)) == 7:
                    time = decimal.Decimal(str(self.trans.header.stamp.secs) + ".00" + str(self.trans.header.stamp.nsecs))
                elif len(str(self.trans.header.stamp.nsecs)) == 8:
                    time = decimal.Decimal(str(self.trans.header.stamp.secs) + ".0" + str(self.trans.header.stamp.nsecs))
                else:
                    time = decimal.Decimal(str(self.trans.header.stamp.secs) + "." + str(self.trans.header.stamp.nsecs))
                diff_time = float(time - self.time_initial)
                if not act:
                    print('time', diff_time)
                ...
                # get at least one past point
                if i > 1:
                    vel = np.abs((trans_shift - trans_past_shift) / (diff_time - trans_past_time))
                    if not act:
                        print('velocity', vel)
                    ...
                    if act and vel == 0.0 and trans_pose[2] < 0.03:
                        # if you have one option go to robot
                        if B[-1][0][0] > 0.8:
                            0  # self.robot_not([-0.2, 0.02, 0.05])
                        elif B[-1][0][1] > 0.8:
                            self.robot_careful([-0.2, 0.02, -0.01], active_trans, self.trans)
                        close = True

                trans_past_shift = trans_shift
                trans_past_time = diff_time
            ...
            if active_cup and active_robot:
                ...
                if diff[0] > 0.75 or diff[2] < 0.0:
                    ...
                    if not found_object:
                        ...
                        hand[0] = hand[0] + fine_tune[0]  #
                        hand[1] = hand[1] + fine_tune[1]  # + 0.05
                        hand[2] = hand[2] + fine_tune[2]  # + 0.18
                        found_object = True
                    else:
                        self.set_pose(hand)
                        self.get_robot_pos()
    ...
    def robot_careful(self, fine_tune, active_object, object_loc):
        # go to cup
        leave = False
        found_object = False
        while not leave:
            if active_object and active_robot:
                cup = object_loc.pose.position
                kuka = self.kuka.pose.position
                ...
                if diff[0] > 0.75 or diff[2] < 0.0:
                    ...
                    if not found_object:
                        hand = diff
                        # raise the gripper a bit higher and take into account the gripper
                        hand[0] = hand[0] + fine_tune[0] #- 0.2
                        hand[1] = hand[1] + fine_tune[1] #+ 0.02
                        hand[2] = hand[2] + fine_tune[2] #+ 0.02
                        found_object = True
                    else:
                        self.set_pose(hand)
                        self.get_robot_pos()
                        if self.ee_real_logged:
                            ...
                            if (np.round(np.linalg.norm(self.target_position - self.end_effector_position), 2)) < 0.01:
                                leave = True
    ...
    def tf_coke(self, data):
        ...
        if once_3:
            last_coke = data
            # set initial position of cup
            self.coke_initial = data.pose.position
            if len(str(data.header.stamp.nsecs)) == 7:
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + ".00" + str(data.header.stamp.nsecs))
            elif len(str(data.header.stamp.nsecs)) == 8:
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + ".0" + str(data.header.stamp.nsecs))
                ...
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + "." + str(data.header.stamp.nsecs))
            once_3 = False
        if data.pose != last_coke.pose:
            last_coke = data
            active_coke = True
            self.coke = data
        elif data.pose == last_coke.pose:
            active_coke = False

    def tf_trans(self, data):
        global last_trans
        global active_trans
        global once_4
        ...
        if once_4:
            last_trans = data
            # set initial position of cup
            self.trans_initial = data.pose.position
            if len(str(data.header.stamp.nsecs)) == 7:
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + ".00" + str(data.header.stamp.nsecs))
            elif len(str(data.header.stamp.nsecs)) == 8:
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + ".0" + str(data.header.stamp.nsecs))
                ...
                self.time_initial = decimal.Decimal(str(data.header.stamp.secs) + "." + str(data.header.stamp.nsecs))
            once_4 = False
        if data.pose != last_trans.pose:
            last_trans = data
            active_trans = True
            self.trans = data
        elif data.pose == last_trans.pose:
            active_trans = False
