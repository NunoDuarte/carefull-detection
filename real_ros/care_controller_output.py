#!/usr/bin/env python
# this version works on python3 ubuntu 20 and python2.7 ubuntu 18?
...
class CareControllerOutput:

    def __init__(self):
        # Subscribers
        self.robot_base = rospy.Subscriber("/Cup_1/pose", PoseStamped, self.tf_base)
        self.red_cup = rospy.Subscriber("/Trans_cup/pose", PoseStamped, self.tf_redcup, queue_size=1)
        ...
        self.epsilon = 0.08 #0.5 # adaptation rate
    ...
    def model(self, ):
        ...
        while not close:
            ...
            if active_cup and active_robot:
                ...
                # get the absolute time and count the time passed from the moment the object appears
                ...
                diff = np.zeros(3)
                diff[0] = np.abs(np.round(kuka.x - cup.x, 2))
                diff[1] = np.abs(np.round(kuka.y - cup.y, 2))
                diff[2] = np.abs(np.round(cup.z - kuka.z, 2))
                distance = math.sqrt(diff[0]**2 + diff[1]**2 + diff[2]**2)
                print('distance', distance)

                # get at least one past point
                if i > 1:
                    vel = np.abs(np.round((cup_shift - cup_past_shift)/float(diff_time - cup_past_time), 1))
                    print('velocity', vel)
                    ...
                    if len(Data) == 40:
                        ...
                        if is_okay:
                            print(B[-1])
                            #                           only B[0]           # only B[1]
                            # z = zip(Data, Data_vel, [row[0][0] for row in B], [row[0][1] for row in B])
                            current_date = datetime.datetime.now()
                            filename = str(current_date.time()) + str(current_date.day) + str(current_date.month) + str(current_date.year)
                            np.savetxt(filename+'.csv', np.c_[[row[0][0] for row in B], [row[0][1] for row in B]], fmt='%1.3f',
                                       delimiter=" ,")
                            with open(filename+'.csv', 'ab') as csvfile:
                                np.savetxt(csvfile, np.c_[Data, Data_vel], fmt='%1.3f', delimiter=" ,")
        ...
        # fileE.write(str(E) + str('\n'))
        # fileB.write(str(B) + str('\n'))
